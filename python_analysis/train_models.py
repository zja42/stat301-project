...
def load_data() -> tuple[pd.DataFrame, pd.Series]:
    ...
    data = data.loc[data["Region"] != 1].copy()
...
def build_models(columns: list[str]) -> dict[str, Pipeline]:
    ...
    estimators = {
        ...
        "Ordinary logistic regression": LogisticRegression(
...
def save_class_balance(y: pd.Series) -> None:
    ...
    ax.set_title(
        f"Class balance in {len(y):,} shopping sessions", loc="left", weight="bold"
    )
...
This project compared {len(cv_results)} classification approaches on **{len(y):,} online
...
def main() -> None:
    ...
    summary = {
        ...
        "source_filter": "Region != 1 (aligned with prediction-branch final project)",
    }
